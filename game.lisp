...
(defun send-state (ball stream)
  (nibbles:write-sb64/be (round (x ball)) stream)
  (nibbles:write-sb64/be (round (y ball)) stream)
  (nibbles:write-sb64/be (round (x-speed ball)) stream)
  (nibbles:write-sb64/be (round (y-speed ball)) stream)
  (force-output stream))

(defun receive-state (ball stream)
  (setf (x ball) (- (width *field*) (nibbles:read-sb64/be stream)))
  (setf (y ball) (nibbles:read-sb64/be stream))
  (setf (x-speed ball) (- (nibbles:read-sb64/be stream)))
  (setf (y-speed ball) (nibbles:read-sb64/be stream)))
